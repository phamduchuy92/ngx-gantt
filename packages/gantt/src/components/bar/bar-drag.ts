...
export class GanttBarDrag implements OnDestroy {
    ...
    private barBeforeHandleDragMove() {
        ...
        if (width > dragMinWidth && diffs > 0) {
            ...
            this.item.updateDate(this.item.end, this.item.end);
        }
    ...
    private barAfterHandleDragMove() {
        ...
        if (width > dragMinWidth && diffs > 0) {
            ...
            this.item.updateDate(this.item.start, this.item.start);
        }
        this.barHandleDragMoveRecordDiffs = diffs;
        this.dragContainer.dragMoved.emit({ item: this.item.origin });
    }
    ...
    private startOfBarHandle() {
        ...
        return {
            ...
            minRangeWidthWidth: this.ganttUpper.view.getDateRangeWidth(this.item.end.startOfDay(), this.item.end)
        };
    }
